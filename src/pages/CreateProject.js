...
const CreateProject = () => {
  ...
  const [showProjectCongratsModal, setCreateProjectCongratsModal] =
    useState(true);
  ...
  const onExpectedLaunchDateChange = (e) => {
    setExpectedLaunchDate(
      parseInt((new Date(e.target.value).getTime() / 1000).toFixed(0))
    );
    console.log(
      parseInt((new Date(e.target.value).getTime() / 1000).toFixed(0)),
      "======================"
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(
      createProjectConfigError,
      "!!!when you creat project, you are wrong"
    );
  };
      ...
      {isSuccess && (
        ...
        >
          <CongratsModalWrapper>
            ...
            <button
              onClick={() => {
                setCreateProjectCongratsModal(false);
                navigateToHome();
              }}
              className="bg-Pure-White text-Pure-Black text-sm font-medium rounded-xl py-2 px-6"
            >
